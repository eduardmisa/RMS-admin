<template>
  ...
  >
    <v-card :loading="loading" v-if="!updated">
      ...
      <v-card-title>
        <span class="mr-3">Update Module</span>
        <v-spacer></v-spacer>
        <div class="mt-2 mb-1">
      ...
      <v-card-subtitle>
        Update details of <br> this module
        <v-spacer></v-spacer>
      </v-card-subtitle>
  ...
  export default {
    ...
    methods: {
      ...
      async Update () {
        ...
        let response = await app.$api.ModuleService.Update(this.moduleId, app.details)
