...
router.post('/login',
 async function(req, res, next) {
  ...
  let url = `${process.env.GATEWAY_HOST}/api/v1/auth/login/`
...
router.get('/current-user',
 async function(req, res, next) {
  ...
  let url = `${process.env.GATEWAY_HOST}/api/v1/auth/current-user/`
...
router.get('/current-user/scope',
 async function(req, res, next) {
  ...
  let url = `${process.env.GATEWAY_HOST}/api/v1/auth/current-user/scope/`
  let token = req.headers.authorization ? req.headers.authorization.replace("Bearer ", "") : ""
